...
function ApproveDepositButton({ investor }: { investor: InvestorRow }) {
  ...
  if (done) {
    return <span className="text-xs text-[#22c55e]">승인 완료</span>
  }
  ...
  if (!showForm) {
    return (
      <button
        ...
        className="text-xs text-[#3d8ef8] hover:underline mr-3"
      >
        입금 승인
      </button>
    )
...
function MintTokensButton({ investor }: { investor: InvestorRow }) {
  ...
  if (done) {
    return <span className="text-xs text-[#22c55e]">발행 완료</span>
  }

  async function handleMint() {
    setLoading(true)
    try {
      // 토큰 발행 확인: tokenMinted = true 로 업데이트
...
function ApproveWithdrawButton({ requestId }: { requestId: string }) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const [done, setDone] = useState(false)
              ...
              <thead>
                <tr className="border-b border-white/[0.07]">
                  {['이름', '이메일', '투자금 (KRW)', '토큰수', '상태', '가입일', '액션'].map((h) => (
                    <th
                      key={h}
                      className="px-5 py-2.5 text-left text-[11px] text-white/[0.28] uppercase tracking-[0.3px] font-medium"
                    >
                      {h}
                    </th>
                  ))}
                ...
                {investors.map((inv) => (
                  <tr key={inv.id} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                    ...
                    <td className="px-5 py-3 text-white">
                      {inv.investment?.amountKrw
                        ? inv.investment.amountKrw.toLocaleString('ko-KR')
                        : '—'}
                    </td>
                    <td className="px-5 py-3 text-white/70">
                      {inv.investment?.tokenAmount
                        ? Number(inv.investment.tokenAmount).toLocaleString('ko-KR', {
                    ...
                    <td className="px-5 py-3">
                      <span
                        className={`text-xs rounded px-2 py-0.5 ${
                          STATUS_STYLE[inv.investment?.status ?? ''] ??
                    ...
                    <td className="px-5 py-3">
                      {inv.investment && !inv.investment.depositConfirmed && (
                        <ApproveDepositButton investor={inv} />
                      )}
                      {inv.investment?.depositConfirmed && !inv.investment.tokenMinted && (
                        <MintTokensButton investor={inv} />
                      )}
                      {inv.investment?.tokenMinted && (
                        <span className="text-xs text-white/30">활성</span>
                      )}
                    </td>
                  </tr>
                ))}
        ...
        <div className="bg-[#0e1425] border border-white/[0.07] rounded-xl overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[600px]">
              <thead>
                <tr className="border-b border-white/[0.07]">
                  {['투자자', '토큰수', '지급 예정 (KRW)', 'Tx 해시', '상태', '신청일', '액션'].map(
                    (h) => (
                      <th
                        key={h}
                        className="px-5 py-2.5 text-left text-[11px] text-white/[0.28] uppercase tracking-[0.3px] font-medium"
              ...
              <tbody>
                {withdrawRequests.map((req) => (
                  <tr key={req.id} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                    ...
                    <td className="px-5 py-3 text-white">
                      {Number(req.tokenAmount).toLocaleString('ko-KR', {
                        maximumFractionDigits: 2,
                      })}
                    </td>
                    <td className="px-5 py-3 text-white">
                      {req.krwAmount.toLocaleString('ko-KR')}
                    </td>
                    <td className="px-5 py-3 text-white/40 font-mono text-xs">
                      {req.txHash ? `${req.txHash.slice(0, 10)}…` : '—'}
                    </td>
                    <td className="px-5 py-3">
                      <span
                        className={`text-xs rounded px-2 py-0.5 ${
                          STATUS_STYLE[req.status] ?? 'bg-white/[0.06] text-white/50'
                        }`}
                      >
                        {req.status}
                      </span>
                    ...
                    <td className="px-5 py-3">
                      {req.status === 'PENDING' && (
                        <ApproveWithdrawButton requestId={req.id} />
                      )}
                    </td>
                  </tr>
                ))}
