...
const STATUS_CONFIG: Record<string, { label: string; className: string }> = {
  ...
  APPROVED:  { label: 'Approved',  className: 'bg-[#3d8ef8]/10 text-[#3d8ef8]' },
  COMPLETED: { label: 'Completed', className: 'bg-[#22c55e]/10 text-[#22c55e]' },
  ...
  if (loading) {
    return (
      <div className="bg-[#0e1425] border border-white/[0.07] rounded-xl p-5 mt-4">
        <div className="text-[11px] text-white/[0.28] uppercase tracking-[0.5px] mb-3">Withdrawal history</div>
              ...
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-sm text-white font-medium">
                      {Number(rec.tokenAmount).toLocaleString(undefined, { maximumFractionDigits: 2 })} TOKEN
                    </span>
                  ...
                  <div className="flex items-center gap-3 text-[11px] text-white/30">
                    <span>{rec.krwAmount.toLocaleString()} KRW</span>
                    <span>·</span>
                ...
                {rec.processedAt && (
                  <div className="text-[11px] text-white/[0.22] shrink-0 ml-4">
                    Processed: {new Date(rec.processedAt).toLocaleDateString()}
                  </div>
                )}
              </div>
            )
